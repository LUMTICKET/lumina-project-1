...
export default function BusinessProfileForm({
      ...
      : [
          {
            id: '1',
            fullName: '',
            role: type === 'individual' ? 'Owner' : 'Director',
            email: '',
            phone: '',
            nationalIdNumber: '',
          },
  ...
  const handleSave = async () => {
    if (
      ...
      Alert.alert(
        'Missing fields',
        'Please fill in all required business information.'
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        'Save failed',
        err?.message || 'Could not save your business profile.'
      );
    } finally {
      setSaving(false);
    }
    ...
    <View style={[styles.wrap, { backgroundColor: colors.bg }]}>
      ...
      >
          ...
          <View style={styles.typeToggle}>
            {(['individual', 'company'] as BusinessType[]).map((t) => (
              ...
              >
                <Text
                  style={{
                    color: type === t ? colors.black : colors.ink,
        ...
        <View
          style={[
            styles.card,
            { backgroundColor: colors.surface, gap: spacing(3) },
          ]}
        >
          <FormField label="Business Name *" colors={colors}>
            <TextInput
              placeholder="e.g. Captain Tours"
          ...
          <FormField label="Trading Name (optional)" colors={colors}>
            <TextInput
              placeholder="e.g. Captain Bus Express"
              placeholderTextColor={colors.inkMuted}
              value={tradingName}
          ...
          <FormField label="Category" colors={colors}>
            <TextInput
              placeholder="e.g. Transport, Events, Tourism"
              placeholderTextColor={colors.inkMuted}
              value={category}
        ...
        {executives.map((exec, idx) => (
          ...
          >
            ...
            <View style={styles.row}>
              <FormField label="Full Name" colors={colors} style={{ flex: 1 }}>
                <TextInput
                  ...
                  placeholderTextColor={colors.inkMuted}
                  value={exec.fullName}
                  onChangeText={(v) => updateExecutive(exec.id, 'fullName', v)}
                  style={[styles.input, inputBase]}
                />
              </FormField>
              <View style={{ width: spacing(3) }} />
              <FormField label="Role" colors={colors} style={{ flex: 1 }}>
                <TextInput
                  placeholder={type === 'individual' ? 'Owner' : 'Director'}
                  placeholderTextColor={colors.inkMuted}
                  value={exec.role}
                  onChangeText={(v) => updateExecutive(exec.id, 'role', v)}
                  editable={type !== 'individual'}
                  style={[
                    styles.input,
                    inputBase,
                    type === 'individual' && { opacity: 0.6 },
                  ]}
                />
              </FormField>
            </View>
            <View style={styles.row}>
              <FormField label="Email" colors={colors} style={{ flex: 1 }}>
                <TextInput
                  placeholder="exec@email.com"
                  placeholderTextColor={colors.inkMuted}
                  value={exec.email}
                  onChangeText={(v) => updateExecutive(exec.id, 'email', v)}
                  autoCapitalize="none"
                  keyboardType="email-address"
                  style={[styles.input, inputBase]}
                />
              ...
              <FormField label="Phone" colors={colors} style={{ flex: 1 }}>
                <TextInput
                  ...
                  placeholderTextColor={colors.inkMuted}
                  value={exec.phone}
                  onChangeText={(v) => updateExecutive(exec.id, 'phone', v)}
                  keyboardType="phone-pad"
                  style={[styles.input, inputBase]}
                />
              </FormField>
            ...
            <FormField label="National ID Number" colors={colors}>
              <TextInput
                ...
                placeholderTextColor={colors.inkMuted}
                value={exec.nationalIdNumber}
                onChangeText={(v) =>
                  updateExecutive(exec.id, 'nationalIdNumber', v)
                }
                style={[styles.input, inputBase]}
              />
            </FormField>
          ...
          >
                  ...
                  {DOC_TYPE_OPTIONS.map((opt) => (
                    <Pressable
                      ...
                      style={[
                        ...
                        {
                          backgroundColor:
                            doc.type === opt.key ? colors.gold : 'transparent',
                        },
                      ]}
                    >
                      <Text
                        style={{
                          fontSize: 12,
                          color:
                            doc.type === opt.key ? colors.black : colors.ink,
                          fontFamily: fontFamilies.bodySemi,
                        }}
                      >
                        {opt.label}
            ...
            <FormField label="Document Title" colors={colors}>
              <TextInput
                ...
                placeholderTextColor={colors.inkMuted}
                value={doc.title}
                onChangeText={(v) => updateDocument(doc.id, 'title', v)}
                style={[styles.input, inputBase]}
              />
            </FormField>
            <Pressable
